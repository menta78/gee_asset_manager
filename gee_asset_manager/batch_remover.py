...
def delete(asset_path):
   ...
   #all_assets_names = list(map(lambda s: s.replace('projects/earthengine-legacy/assets/', ''), all_assets_names))
...
def __delete_recursive(asset_path):
    ...
    if not info:
        ...
        sys.exit(1)
    elif info['type'].casefold() == 'Image'.casefold():
        pass
    elif info['type'].casefold() == 'Folder'.casefold():
